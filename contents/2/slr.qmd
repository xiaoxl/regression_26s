...
In order to find the best parameters, we need to minimize the sum of square errors 
$$
...
$$
\begin{split}
\pdv{L}{\beta_0}&=\sum_{i=1}^n2(y_i-\beta_0-\beta_1x_i)(-1)=-\sum_{i=1}^ny_i-n\beta_0-\beta_1(\sum_{i=1}^nx_i)=-n(\bar y-\beta_0-\beta_1\bar x)=0,\\
...
\end{split}
$$
Take all data into consideration. Let $\mathbf x=(x_1,\ldots,x_n)$ and $\mathbf y=(y_1,\ldots,y_n)$. Assuming that all $\varepsilon_i$'s be independent, then the likelihood function is
$$
...
:::
Similar to the sample variance scenario, the unbiased estimator of the error variance is 
$$
s^2=\frac{SSE}{n-2}.
$$
