...
Given some data, we would like to know whether these data are "exotic" enough--under the assumption that the null hypothesis $H_0$ is true--to justify rejecting $H_0$. In other words, we compute the probability of obtaining data at least as extreme as what we observed when assuming $H_0$ is true. This probability is called the p-value. Once the p-value is smaller than the chosen level of significance $\alpha$, we reject $H_0$. 
...
$$
is the p-value for a right-tailed test.
...
<!-- In many scientific contexts, the emphasis is on avoiding false claims of an effect. Thus, researchers often prefer to minimize Type I errors even at the cost of accepting a higher risk of Type II errors. In other words, it is usually considered worse to declare an effect that does not exist than to miss an effect that actually exists. -->
