...
When $R_j^2$ is bigger, the variance will be amplified. In other words, if one variable depends on other variables, which means that $R_j^2$ is high, the variance of $\hat\beta_j$ is also very high. This lemma shows that when there are (almost) multiconlinear variables, the varaiance of the corresponding estimator will be extremely large.
