...
|------|------------------|
| $y = \beta_0 + \beta_1 x_1 + \varepsilon$ | • Simple linear regression  <br> • Simple first-order model with one predictor |
| $y = \beta_0 + \beta_1 x_1 + \beta_2 x_2 + \beta_3 x_3 + \varepsilon$ | • Multiple linear regression with three predictors <br> • First-order model with three predictors |
...
4. Specify the distribution of $\varepsilon$ and estimate its $\sigma^2$.
5. Evaluate the utility of the model, like checking p-values and $R^2$.
6. Verify the model by checking the assumptions on $\varepsilon$.
...
1. For any given $x_i$, $\varepsilon\sim \distnormal(0,\sigma^2)$.
...
::: {#lem-vif}
$$
\Var(\hat\beta_j)\propto\frac{1}{1-R^2_j}.
$$
...
Let the model be $y=X\beta+\varepsilon$. Let the j-th predictor column be $x_j$ and $X_{-j}$ be the matrix of all other predictor columns. Rewrite the design matrix as 
$$
X=\mqty[x_j&X_{-j}].
$$
...
Then 
$$
X^TX=\mqty[x_j^Tx_j&x_j^TX_{-j}\\X^T_{-j}x_j&X^T_{-j}X_{-j}].
$$

Consider 
$$
\mqty[A&B\\C&D]=\underbrace{\mqty[I&BD^{-1}\\0&I]}_{(I)}\underbrace{\mqty[A-BD^{-1}C&0\\C&D]}_{(II)}.
$$
Let $A-BD^{-1}C=S$. Then 
$$
(I)^{-1}=\mqty[I&-BD^{-1}\\o&I],\quad (II)^{-1}=\mqty[S^{-1}&0\\-D^{-1}CS^{-1}&D^{-1}],
$$
so
$$
\mqty[A&B\\C&D]^{-1}=(II)^{-1}(I)^{-1}=\mqty[S^{-1}&0\\-D^{-1}CS^{-1}&D^{-1}]\mqty[I&-BD^{-1}\\0&I]=\mqty[S^{-1}&-S^{-1}BD^{-1}\\-D^{-1}CS^{-1}&D^{-1}(CS^{-1}BD^{-1}+I)]
$$
Therefore the left upper corner of the inverse matrix is $S^{-1}=(A-BD^{-1}C)^{-1}$. Then
$$
(X^TX)^{-1}_{jj}=(x_j^Tx_j-x^T_jX_{-j}(X^T_{-j}X_{-j})^{-1}X^T_{-j}x_j)^{-1}.
$$
Let $P_j=X_{-j}(X^T_{-j}X_{-j})^{-1}X^T_{-j}$. We have
$$
\begin{split}
\Var(\hat\beta_j)&=\mqty[(X^TX)^{-1}\sigma^2]_{jj}=(X^TX)^{-1}_{jj}\sigma^2\\
&=\frac{\sigma^2}{x_j^Tx_j-x^T_jP_jx_j}=\frac{\sigma^2}{x_j^T(I-P_j)x_j}.
\end{split}
$$


On the other side, use $x_j=P_jX_{-j}+\varepsilon$. Then $x_j^T(I-P_j)x_j=SSE_j$. Since $1-\frac{SSE_j}{SST_j}=R_j^2$, we have
$$
x_j^T(I-P_j)x_j = SST_j(1-R_j^2).
$$
So 
$$
\begin{split}
\Var(\hat\beta_j)&=\frac{\sigma^2}{x_j^T(I-P_j)x_j}=\frac{\sigma^2}{SST_j}\frac{1}{1-R_j^2}\propto\frac{1}{1-R_j^2}.
\end{split}
$$
...
### Dependence and Multicolinearity
...
Note that
$$
R^2=\cor(X^2,X)^2=\frac{\Cov(X^2, X)^2}{\Var(X^2)\Var(X)}=\frac{(\Exp[X^3]-\Exp[X]\Exp[X^2])^2}{\Var(X^2)\Var(X)}.
$$

This result depends on the distribution of $X$. For example, if the distribution of $X$ is symmetric, then $\Exp[X]=0$ and $\Exp[X^3]=0$. Therefore $R^2(X^2\sim X)=0$. 
...
More generally when $\mu$ can be $0$,
$$
\begin{split}
\Exp[X]&=\mu,\\
\Exp[X^2]&=\sigma^2+\mu^2,\\
\Exp[X^3]&=\mu^3+3\mu\sigma^2,\\
\Exp[X^4]&=\mu^4+6\mu^2\sigma^2+3\sigma^4,\\
\Cov(X,X^2)&=\Exp[X^3]-\Exp[X]\Exp[X^2]=(\mu^3+3\mu\sigma^2)-\mu(\sigma^2+\mu^2)=2\mu\sigma^2,\\
\Var[X]&=\sigma^2,\\
\Var[X^2]&=\Exp[X^4]-\Exp[X^2]^2=(\mu^4+6\mu^2\sigma^2+3\sigma^4)-(\sigma^2+\mu^2)^2=4\mu^2\sigma^2+2\sigma^4,\\
\cor(X,X^2)&=\frac{2\mu}{\sqrt{4\mu^2+2\sigma^2}}.
\end{split}
$$
So $R^2(X^2\sim X)=\frac{2\mu^2}{2\mu^2+\sigma^2}$. Then the corresponding VIF is $VIF(X^2\sim X)=1+2\frac{\mu^2}{\sigma^2}$.
...
\begin{split}
\Exp[X] &= \frac{1}{\lambda},\\
\Exp[X^2] &= \frac{2}{\lambda^2},\\
\Exp[X^3] &= \frac{6}{\lambda^3},\\
\Exp[X^4] &= \frac{24}{\lambda^4}.
\end{split}
$$
...
\Var(X^2)
&=\Exp[X^4]-\Exp[X^2]^2\\
&=\frac{24}{\lambda^4}-\frac{4}{\lambda^4}\\
&=\frac{20}{\lambda^4}.
\end{split}
$$
...
\begin{split}
R^2
&=\frac{\Cov(X,X^2)^2}{\Var(X)\Var(X^2)}\\
&=\frac{(4/\lambda^3)^2}{(1/\lambda^2)(20/\lambda^4)}\\
&=\frac{16}{20}\\
&=0.8.
\end{split}
$$
...
R^2
&=\frac{\Cov(X,X^2)^2}{\Var(X)\Var(X^2)}\\
&=\frac{(1/12)^2}{(1/12)(4/45)}\\
&=\frac{1}{12}\cdot\frac{45}{4}\\
&=\frac{15}{16}.
\end{split}
$$

Thus $R^2(X^2\sim X)=\frac{15}{16}=0.9375$. Then $VIF(X^2\sim X)=16$.
</details>

:::


Here is a table of common distributions. 
...
::: {.callout-note}
One common trick to reduce multicollinearity is to center the variable:
$$
z = x - \bar x.
$$
Then $z$ is centered around $0$, which often makes $z$ and $z^2$ less linearly dependent than $x$ and $x^2$. This transformation can reduce collinearity among predictors, while leaving the fitted values, residuals, and $R^2$ of the model unchanged.
:::
...
::: {#def-}
- The effect of variable $x$ is the partial derivative of the expected response with respect to $x$:
$$
\text{effect of }x=\pdv{\Exp(y\mid \vb x)}{x}.
$$
Equivalently, it measures how much $\Exp(y)$ changes when $x$ increases by $1$, holding other variables fixed.
...
When deciding which interaction terms to consider, a common idea is to ask whether the influence of one variable on the response may change depending on another variable. Such interactions are often reasonable when two predictors describe related aspects of the same phenomenon, when scientific knowledge suggests a combined effect, or when graphical exploration shows that the slope of one predictor appears different at different levels of another predictor. In these situations, adding an interaction term allows the model to capture how the relationship between a predictor and the response varies across conditions.
...
$$
we obtain the following interpretations:
...
Equivalently, 
$$
\beta_3=\frac{\partial^2 \Exp(y)}{\partial x_1\partial x_2},
$$
so $\beta_3$ measures how quickly one slope changes with the other predictor.
:::

We may still write
$$
\beta_1=\text{effect of }x_1\text{ when }x_2=0.
$$

However in many cases, $x_2=0$ is not meaningful in practice. To obtain a more useful interpretation, we center the variable:
$$
x_2^*=x_2-\bar x_2,\quad \beta_0^*=\beta_0+\beta_2\bar x_2,\quad \beta_1^*=\beta_1+\beta_3\bar x_2.
$$

Then the orginal model becomes
$$
\Exp(y)=\beta_0^*+\beta_1^*x_1+\beta_2x_2^*+\beta_3x_1x_2^*.
$$
In this case, $x_2^*$ can be $0$. Then main effects represent slopes at the average level of the other predictor. We call $\beta_1^*$ centralized main effect.
...
When considering higher-degree terms, we ask whether the relationship between a predictor and the response shows curvature. If exploratory plots or prior knowledge suggest a nonlinear pattern, polynomial terms can be added to allow the model to capture this curvature.

::: {#def-}

A higher-order term is a nonlinear transformation of a predictor, such as $x^2$, $x^3$, $\sqrt{x}$, or $\log x$.
:::

::: {.callout-note}
Recall that the effect of a variable $x$ is the rate of change of the expected response with respect to $x$:
$$
\text{effect of }x=\pdv{\Exp(y\mid \vb x)}{x}.
$$
With higher-degree terms, the coefficient of $x$ is generally not the effect of $x$; the effect depends on the value of $x$.
:::
...
| :--- | :--- | :--- |
| `A + B` | Main Effects | $y = \beta_0 + \beta_1 A + \beta_2 B$ (Independent effects) |
| `A : B` | Interaction Term | $y = \beta_0 + \beta_1 (AB)$ (Only the interaction) |
| `A * B` | Full Factorial | $y = \beta_0 + \beta_1 A + \beta_2 B + \beta_3 (AB)$ |
| `(A + B + C)^2` | Degree Limit | All main effects and all possible 2-way interactions |
| `I()` | Math Identity | Wraps arithmetic: $I(A^2)$ or $I(A + B)$ as a single term |
| `1` | Intercept | The constant $\beta_0$ (included by default) |
| `0 or -1` | No Intercept | Removes $\beta_0$, forcing the model through the origin |
...
- Quadratic Model: `y ~ x + I(x^2)`
- Summation as Predictor: `y ~ I(x1 + x2)`
