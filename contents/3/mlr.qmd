...
## Interpretations of special terms
...
We need to understand the intepretations of both the interaction term $\beta_3$ and the main effect $\beta_1$. Note that
...
On the other side, we could inteprete $\beta_1$ as 
$$
\beta_1=\text{effect of }x_1\text{ when }x_2=0.
$$

However in many cases, $x_2=0$ is not meaningful. Therefore we need to change the formula a little bit to understand $\beta_1$. Let

$$
x_2^*=x_2-\bar x_2,\quad \beta_0^*=\beta_0+\beta_2\bar x_2,\quad \beta_1^*=\beta_1+\beta_3\bar x_2.
$$

Then the orginal model is
$$
...
> The term $x^2$ captures curvature; the effect of $x$ changes as $x$ changes.

---

- Interpretation via derivative (changing slope)

$$
\frac{dE(y)}{dx}=\beta_1+2\beta_2x.
$$
...
- If $\beta_2>0$: U-shaped  
- If $\beta_2<0$: inverted U

---

- Turning point (very useful in teaching)
...
$$
\text{performance}\sim \text{stress}+\text{stress}^2
$$

Interpretation:

- moderate stress best  
- too little or too much worse

---

- Interaction vs Higher-Degree: Key Difference
...
👉 Interaction = cross-variable change  
👉 Polynomial = within-variable change  
...
> Higher-degree: “effect changes as the variable changes.”
...
| `- B` | Exclusion | Removes a specific term (e.g., `y ~ . - x1`) |
